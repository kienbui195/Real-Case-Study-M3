        ...
        connection.connect(() => {
            ...
            connection.query(sql, (err, results) => {
                ...
                for (let i = 0; i < results.length; i++) {
                    ...
                    html += `<td><a href='/customer/addcart?id=${results[i].pro_id}' type="button" class="btn btn-success">Thêm vào giỏ hàng</a></td>`
                    html += `</tr>`;
                }
    ...
    create(req, res) {
        if (req.method === "GET") {
            ...
            req.on('end', () => {
                let newData = qs.parse(data);
                connection.connect(() => {
                    const sql = `INSERT INTO product (name, price, quantityInStock, description)
                                 VALUES ('${newData.nameProduct}', ${+newData.priceProduct},
                                         ${+newData.quantityProduct}, '${newData.description}')`
                    connection.query(sql, (err, result) => {
                        if (err) {
                            console.log(err)
                        }
